...
// =============================================================================
// BILDERGALERIE & DRAG & DROP UPLOAD
// =============================================================================
...
/**
 * Initialisiert Drag & Drop für Bildupload
 */
function initImageUpload() {
    // Add Modal Dropzone
    const addDropzone = document.getElementById('add-image-dropzone');
    const addFileInput = document.getElementById('obj-images');
    
    if (addDropzone && addFileInput) {
        setupDropzone(addDropzone, addFileInput, handleAddImageSelect);
    }
    
    // Edit Modal Dropzone
    const editDropzone = document.getElementById('edit-image-dropzone');
    const editFileInput = document.getElementById('edit-obj-images');
    
    if (editDropzone && editFileInput) {
        setupDropzone(editDropzone, editFileInput, handleEditImageUpload);
    }
}

/**
 * Richtet eine Dropzone ein
 */
function setupDropzone(dropzone, fileInput, callback) {
    // Klick auf Dropzone öffnet File-Dialog
    dropzone.addEventListener('click', () => fileInput.click());
    
    // File Input Change
    fileInput.addEventListener('change', (e) => {
        callback(e.target.files);
        e.target.value = ''; // Reset für erneutes Upload
    });
...
/**
 * Behandelt Bildauswahl im Add Modal
 */
function handleAddImageSelect(files) {
    const previewContainer = document.getElementById('add-image-preview');
    if (!previewContainer) return;
    
    // Validiere und füge zu selectedFiles hinzu
    Array.from(files).forEach(file => {
        if (validateImageFile({ 
            ...
            showNotification(`${file.name}: Ungültiges Bild (Max 2MB, nur JPG/PNG)`, 'warning');
        }
    });
}

/**
 * Erstellt ein Preview-Item
 */
function createPreviewItem(src, index) {
    ...
    item.innerHTML = `
        ...
        <button type="button" class="preview-image-remove" onclick="removePreviewImage(${index})">×</button>
    `;
...
function removePreviewImage(index) {
    ...
    selectedFiles.forEach((file, i) => {
        const reader = new FileReader();
        reader.onload = (e) => {
            const previewItem = createPreviewItem(e.target.result, i);
            previewContainer.appendChild(previewItem);
        };
        reader.readAsDataURL(file);
    });
...
/**
 * Behandelt Bildupload im Edit Modal (direkt hochladen)
 */
async function handleEditImageUpload(files) {
    const objectId = document.getElementById('edit-object-id').value;
    if (!objectId) {
        showNotification('Objekt-ID fehlt', 'error');
        return;
    ...
    const formData = new FormData();
    formData.append('action', 'upload_object_images');
    formData.append('object_id', objectId);
    
    Array.from(files).forEach(file => {
        if (validateImageFile({ name: file.name, type: file.type, size: file.size })) {
            formData.append('images[]', file);
        }
    });
    
    try {
        showNotification('Bilder werden hochgeladen...', 'info', 2000);
        ...
        if (result.success) {
            showNotification(`${result.count} Bild(er) erfolgreich hochgeladen`, 'success');
            ...
            // Galerie neu laden
            loadImageGallery(objectId);
        } else {
            showNotification(result.message || 'Fehler beim Hochladen', 'error');
        }
    } catch (error) {
        console.error('Upload error:', error);
        showNotification('Verbindungsfehler beim Hochladen', 'error');
    }
}

/**
 * Lädt die Bildergalerie für ein Objekt
 */
async function loadImageGallery(objectId) {
    const galleryContainer = document.getElementById('edit-image-gallery');
    if (!galleryContainer) return;
    
    try {
        const formData = new FormData();
        formData.append('action', 'get_object_images');
        formData.append('object_id', objectId);
        ...
        if (result.success) {
            ...
            displayImageGallery(result.images, galleryContainer);
        } else {
            galleryContainer.innerHTML = '<p style="color: #999; text-align: center; padding: 1rem;">Keine Bilder vorhanden</p>';
        }
    } catch (error) {
        console.error('Error loading gallery:', error);
        galleryContainer.innerHTML = '<p style="color: #dc3545; text-align: center; padding: 1rem;">Fehler beim Laden</p>';
    }
}

/**
 * Zeigt die Bildergalerie im Edit-Modal an
 */
function displayImageGallery(images, container) {
    if (!container) return;
    
    if (images.length === 0) {
        container.innerHTML = '<p style="color: #999; text-align: center; padding: 1rem; font-style: italic;">Keine Bilder vorhanden</p>';
        return;
    }
    
    container.innerHTML = images.map((img, index) => `
        <div class="gallery-image-item" data-image-id="${img.id}" data-index="${index}" draggable="true">
            <img src="${escapeHtml(img.image_path)}" alt="Bild ${index + 1}">
            <div class="gallery-image-controls">
                <span class="gallery-image-order">${index + 1}</span>
                <button type="button" class="gallery-image-delete" onclick="deleteGalleryImage(${img.id})">×</button>
            </div>
        </div>
    `).join('');
    
    // Drag & Drop für Sortierung
    setupGalleryDragDrop(container);
}

/**
 * Richtet Drag & Drop für Galerie-Sortierung ein
 */
function setupGalleryDragDrop(container) {
    ...
    container.addEventListener('dragend', async function() {
        const newOrder = Array.from(container.querySelectorAll('.gallery-image-item'))
            .map(item => item.dataset.imageId);
        
        const objectId = document.getElementById('edit-object-id').value;
        await saveImageOrder(objectId, newOrder);
    });
}

/**
 * Speichert die neue Bildordnung
 */
async function saveImageOrder(objectId, imageIds) {
    try {
        const formData = new FormData();
        formData.append('action', 'reorder_images');
        formData.append('object_id', objectId);
        imageIds.forEach((id, index) => {
            formData.append(`image_ids[${index}]`, id);
        });
        ...
        if (result.success) {
            showNotification('Reihenfolge gespeichert', 'success', 2000);
            loadImageGallery(objectId); // Neu laden für aktualisierte Nummern
        }
    } catch (error) {
...
async function deleteGalleryImage(imageId) {
    ...
    try {
        const formData = new FormData();
        formData.append('action', 'delete_object_image');
        ...
        if (result.success) {
            showNotification('Bild gelöscht', 'success', 2000);
            const objectId = document.getElementById('edit-object-id').value;
            loadImageGallery(objectId);
        } else {
            showNotification(result.message || 'Fehler beim Löschen', 'error');
...
/**
 * Zeigt Bildergalerie im Info-Panel
 */
function displayInfoPanelGallery(images) {
    ...
    const galleryHtml = `
        <div class="info-panel-gallery">
            <img src="${escapeHtml(images[0].image_path)}" 
                 alt="Bild 1" 
                 class="gallery-main-image" 
                 id="info-gallery-image">
            
            ${images.length > 1 ? `
                <div class="gallery-nav">
                    <button class="gallery-nav-btn" onclick="previousImage()" id="gallery-prev">‹</button>
                    <button class="gallery-nav-btn" onclick="nextImage()" id="gallery-next">›</button>
                </div>
                ...
                <div class="gallery-thumbnails">
                    ${images.map((img, index) => `
                        <div class="gallery-thumbnail ${index === 0 ? 'active' : ''}" 
                             ...
                             data-index="${index}">
                            <img src="${escapeHtml(img.image_path)}" alt="Thumbnail ${index + 1}">
                        </div>
                    `).join('')}
                </div>
            ` : ''}
...
/**
 * Aktualisiert Galerie-Anzeige
 */
function updateGalleryDisplay() {
    const mainImage = document.getElementById('info-gallery-image');
    const counter = document.getElementById('gallery-current');
    const thumbnails = document.querySelectorAll('.gallery-thumbnail');
    const prevBtn = document.getElementById('gallery-prev');
    const nextBtn = document.getElementById('gallery-next');
    
    if (mainImage && currentGalleryImages[currentImageIndex]) {
        mainImage.src = currentGalleryImages[currentImageIndex].image_path;
...
/**
 * Erweitere die bestehende loadObjectDataForPanel Funktion
 */
const originalLoadObjectDataForPanel = loadObjectDataForPanel;
async function loadObjectDataForPanel(objectId, panel, markerElement) {
    // Original-Funktion aufrufen
    await originalLoadObjectDataForPanel(objectId, panel, markerElement);
    ...
    try {
        ...
        if (result.success && result.images.length > 0) {
            ...
            // Füge Galerie zum Panel hinzu (nach dem Titel)
            const galleryHtml = displayInfoPanelGallery(result.images);
            const header = panel.querySelector('.info-panel-header') || 
                          panel.querySelector('div:first-child');
            
            if (header && galleryHtml) {
                header.insertAdjacentHTML('afterend', galleryHtml);
            }
        }
    } catch (error) {
        console.error('Error loading images for info panel:', error);
    }
}

/**
 * Erweitere populateEditModal um Bildergalerie zu laden
 */
const originalPopulateEditModal = populateEditModal;
function populateEditModal(objectData) {
    originalPopulateEditModal(objectData);
    
    // Lade Bildergalerie
    if (objectData.id) {
        loadImageGallery(objectData.id);
    }
}

/**
 * Erweitere handleAddObject um Bilder hochzuladen
 */
const originalHandleAddObject = handleAddObject;
async function handleAddObject(e) {
    if (!e) return;
    e.preventDefault();
    ...
    try {
        ...
        if (result.success) {
            ...
            // Wenn Bilder ausgewählt wurden, hochladen
            if (selectedFiles.length > 0) {
                const imageFormData = new FormData();
                imageFormData.append('action', 'upload_object_images');
                imageFormData.append('object_id', newObjectId);
...
/**
 * Keyboard Navigation für Galerie
 */
document.addEventListener('keydown', function(e) {
    const infoPanel = document.getElementById('info-panel');
    if (!infoPanel || currentGalleryImages.length === 0) return;
...
console.log('Image Gallery System loaded');
